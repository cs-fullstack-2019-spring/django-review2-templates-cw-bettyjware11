...
# def index(request):
#     return HttpResponse("Hello, world. You're at the classworkApp index.")
...
def list_songs(request):
    ...
    return render(request, 'classworkApp/songs.html', context)

# takes parameter
def list_song(request, song_id):
    # song id is the variable we built into the url when we coded the url...
    return render(request, 'classworkApp/song_details.html', {'songs': songs[song_id]})

# returns data for filtered list...
def list_song2(request, song_id):
    return render(request, 'classworkApp/filtered_details.html', {'songs': songs[song_id]})
